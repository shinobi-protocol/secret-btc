...
const typeMap: any = {
    Config: o(
        [
            { json: 'multisigAddress', js: 'multisigAddress', typ: '' },
            { json: 'lcdURL', js: 'lcdURL', typ: '' },
...
export function loadConfig(filePath: string): Config {
    if (existsSync(filePath)) {
        const json: string = readFileSync(filePath, 'utf8');
        return Convert.toConfig(json);
    } else {
        throw new CliError(
            'Cli config is not defined. initialize cli with `init` command at first.'
        );
    }
}
