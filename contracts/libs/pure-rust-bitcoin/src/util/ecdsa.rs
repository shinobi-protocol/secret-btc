...
impl PublicKey {
    ...
    pub fn pubkey_hash(&self) -> PubkeyHash {
        if self.compressed {
            PubkeyHash::hash(&self.key.serialize_compressed())
        } else {
    ...
    pub fn wpubkey_hash(&self) -> Option<WPubkeyHash> {
        if self.compressed {
            Some(WPubkeyHash::from_inner(
                hash160::Hash::hash(&self.key.serialize_compressed()).into_inner()
            ))
        } else {
    ...
    pub fn from_slice(data: &[u8]) -> Result<PublicKey, Error> {
        let (compressed, format) = match data.len() {
            33 => (true, secp256k1::PublicKeyFormat::Compressed),
            65 => (false, secp256k1::PublicKeyFormat::Full),
            len =>  { return Err(base58::Error::InvalidLength(len).into()); },
        };

        Ok(PublicKey {
            compressed: compressed,
            key: secp256k1::PublicKey::parse_slice(data, Some(format))?,
        })
    }
...
impl fmt::Display for PublicKey {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if self.compressed {
            for ch in &self.key.serialize_compressed()[..] {
                write!(f, "{:02x}", ch)?;
            }
        } else {
...
impl PrivateKey {
    ...
    pub fn from_slice(data: &[u8], network: Network) -> Result<PrivateKey, Error> {
        Ok(PrivateKey::new(
            secp256k1::SecretKey::parse_slice(data)?,
            network,
        ))
    }
...
impl ::serde::Serialize for PublicKey {
    fn serialize<S: ::serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        if s.is_human_readable() {
            ...
            if self.compressed {
                s.serialize_bytes(&self.key.serialize_compressed()[..])
            } else {
