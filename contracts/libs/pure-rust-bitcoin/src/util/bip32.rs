...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Error::FromHex(ref err) => write!(f, "from hex error: {}", err),
        }
    }
}
...
impl From<key::Error> for Error {
    fn from(err: key::Error) -> Self {
        match err {
            ...
            key::Error::FromHex(e) => Error::FromHex(e),
...
impl ExtendedPrivKey {
    ...
    pub fn ckd_priv(&self, i: ChildNumber) -> Result<ExtendedPrivKey, Error> {
        ...
        match i {
            ChildNumber::Normal { .. } => {
                ...
                hmac_engine.input(&PublicKey::from_private_key(&self.private_key).key.serialize_compressed()[..]);
            }
        ...
        sk.key.tweak_add_assign(&self.private_key.key)?;
    ...
    pub fn identifier(&self) -> XpubIdentifier {
        ExtendedPubKey::from_private(self).identifier()
    }
...
impl ExtendedPubKey {
    ...
    pub fn from_private(sk: &ExtendedPrivKey) -> ExtendedPubKey {
        ExtendedPubKey {
            ...
            public_key: PublicKey::from_private_key(&sk.private_key),
    ...
    pub fn ckd_pub_tweak(&self, i: ChildNumber) -> Result<(PrivateKey, ChainCode), Error> {
        match i {
            ...
            ChildNumber::Normal { index: n } => {
                let mut hmac_engine: HmacEngine<sha512::Hash> = HmacEngine::new(&self.chain_code[..]);
                hmac_engine.input(&self.public_key.key.serialize_compressed()[..]);
    ...
    pub fn ckd_pub(
        ...
        pk.key.tweak_add_assign(&sk.key)?;
    ...
    pub fn encode(&self) -> [u8; 78] {
        ...
        ret[9..13].copy_from_slice(&endian::u32_to_array_be(u32::from(self.child_number)));
        ret[13..45].copy_from_slice(&self.chain_code[..]);
        ret[45..78].copy_from_slice(&self.public_key.key.serialize_compressed()[..]);
        ret
    }
