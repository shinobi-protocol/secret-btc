...
fn query_current_highest_block_hash<S: Storage>(storage: &S) -> QueryResult {
    let mut light_client_db = StorageLightClientDB::from_readonly_storage(storage);
...
fn query_verify_response_deliver_tx_proof<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
...
mod test {
    ...
    use sfps_lib::cosmos_proto::cosmos::base::abci::v1beta1::TxMsgData;
    ...
    fn test_verify_subsequent_light_blocks() {
        ...
        {
            "verify_subsequent_light_blocks": {
                "current_highest_header": "",
                "light_blocks": [
        ...
        init(&mut deps, mock_env("initializer", &[]), init_msg).unwrap();
        let committed_hashes = match from_binary(&query(&deps, query_msg.clone()).unwrap()).unwrap()
        {
            QueryAnswer::VerifySubsequentLightBlocks { committed_hashes } => committed_hashes,
            _ => unreachable!(),
        };
        let (current_highest_header, light_blocks) = match query_msg {
            QueryMsg::VerifySubsequentLightBlocks {
                current_highest_header,
                light_blocks,
                ..
            } => (current_highest_header, light_blocks),
            _ => unreachable!(),
        };
        assert_eq!(
            committed_hashes.hashes.first_hash,
            hash_header(&current_highest_header)
        );
        for (i, hash) in committed_hashes.hashes.following_hashes.iter().enumerate() {
            assert_eq!(
                hash,
                &header_hash_with_height_of_light_block(&light_blocks[i])
            )
        }

        let query_msg = QueryMsg::VerifySubsequentLightBlocks {
            current_highest_header: current_highest_header.clone(),
            light_blocks: light_blocks.clone(),
        ...
        assert_eq!(
            ...
            vec![
                header_hash_with_height_of_light_block(&light_blocks[0]),
                header_hash_with_height_of_light_block(&light_blocks[2]),
                header_hash_with_height_of_light_block(&light_blocks[4])
            ]
        );
        ...
        let err = &query(&deps, query_msg.clone()).unwrap_err();
        assert_eq!(
            err.to_string(),
        ...
        let query_msg = QueryMsg::VerifySubsequentLightBlocks {
            current_highest_header: {
                let mut correct = current_highest_header.clone();
                correct.height += 1;
                correct
            },
            light_blocks: light_blocks.clone(),
        ...
        assert_eq!(
            err.to_string(),
            "Generic error: invalid current highest header"
        );

        let query_msg = QueryMsg::VerifySubsequentLightBlocks {
            current_highest_header: current_highest_header.clone(),
            light_blocks: {
                let mut lbs = light_blocks.clone();
