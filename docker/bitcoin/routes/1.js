...
let pUtxosByScriptRange = (scId) =>
  new Promise((resolve, reject) => {
    indexd().utxosByScriptRange(
      {
        scId,
        heightRange: [0, 0xffffffff],
        mempool: true,
      },
      DBLIMIT,
      (err, data) => {
...
module.exports = function (router, callback) {
  ...
  router.get("/b/:id/header", bestInjector, hexWare, (req, res) => {
    rpc("getblockheader", [req.params.id, false], (err, json) => {
      if (err && /not found/.test(err.message))
        return res.easy(err, err.message);
      res.easy(err, json);
    });
  });
  ...
  // added by shinobi
  router.get("/b/:id/block", bestInjector, hexWare, (req, res) => {
    rpc("getblock", [req.params.id, 0], (err, data) => {
      if (err && /not found/.test(err.message))
        return res.easy(err, err.message);
      res.easy(err, data);
    });
  ...
  router.post("/r/generate", authMiddleware, (req, res) => {
    rpc("getnewaddress", [], (err, address) => {
      ...
      rpc(
        "generatetoaddress",
        [parseInt(req.query.count) || 1, address],
        res.easy
      );
    });
  });
  ...
  router.post("/r/faucet", authMiddleware, (req, res) => {
    rpc(
      "sendtoaddress",
      [req.query.address, parseInt(req.query.value) / 1e8],
      res.easy
    );
  });
  ...
  router.post("/r/faucetScript", authMiddleware, async (req, res) => {
    try {
      ...
      const txId = await pRpc("sendtoaddress", [
        address,
        (parseInt(req.query.value) * 2) / 1e8,
      ]);
      let unspent;
      let counter = 10;
      while (!unspent) {
      ...
      txvb.addOutput(
        Buffer.from(req.query.script, "hex"),
  ...
  fs.readFile(process.env.KEYDB, (err, buffer) => {
    ...
    debug(
      `imported ${Object.keys(AUTH_KEYS).length} authorized keys`.toUpperCase()
    );
